...
export default Module => {
    ...
    function setValue(ptr, value, type, noSafe) {
        type = type || "i8";
        if (type.endsWith("*")) type = "i32"; // pointers are 32-bit
        switch (type) {
            case "i1":
                HEAP8[ptr >> 0] = value;
                break;
            ...
            case "i64":
                tempI64 = [
                    value >>> 0,
                    (tempDouble = value,
                    Number(Math.abs(tempDouble)) >= 1.0
                        ? tempDouble > 0.0
                            ? (Math.min(Number(Math.floor(tempDouble / 4294967296.0)), 4294967295.0) | 0) >>> 0
                            : ~~Number(Math.ceil((tempDouble - Number(~~tempDouble >>> 0)) / 4294967296.0)) >>> 0
                        : 0),
                ],
    ...
    function stringToAscii(str, outPtr) {
        writeAsciiToMemory(str, outPtr, false);
    }
    ...
    function writeStringToMemory(string, buffer, dontAddNull) {
        ...
        let /** @type {number} */ end, /** @type {number} */
                                  lastChar;
        if (dontAddNull) {
    ...
    function createWasm() {
        ...
        async function instantiateArrayBuffer(receiver) {
            return getBinaryPromise()
                .then(async binary => WebAssembly.instantiate(binary, info))
        ...
        async function instantiateAsync() {
            if (
                ...
                return fetch(wasmBinaryFile, {credentials: "same-origin"}).then(async response => {
                    const result = WebAssembly.instantiateStreaming(response, info);

                    return result.then(receiveInstantiationResult, async reason => {
