...
export const shim = WorldWind => {
    WorldWind.WorldWindow.prototype.computeViewingTransform = function(projection, modelview) {
        if (!modelview) {
            throw new WorldWind.ArgumentError(WorldWind.Logger.logMessage(WorldWind.Logger.LEVEL_SEVERE, "WorldWindow", "computeViewingTransform", "missingModelview"));
        }
        ...
        const lookAtPosition = new WorldWind.Position(navigator.lookAtLocation.latitude, navigator.lookAtLocation.longitude, 0);
        ...
        if (projection) {
            ...
            const globeRadius = WorldWind.WWMath.max(globe.equatorialRadius, globe.polarRadius),
            ...
            let farDistance = (eyeHorizon + atmosphereHorizon) * 4;
            if (farDistance < 1e3) farDistance = 1e3;
            ...
            let nearDistance = WorldWind.WWMath.perspectiveNearDistanceForFarDistance(farDistance, 10, this.depthBits) / 4;
            
            // Prevent the near clip plane from intersecting the terrain.
            const distanceToSurface = eyePos.altitude - globe.elevationAtLocation(eyePos.latitude, eyePos.longitude);
            if (distanceToSurface > 0) {
                const maxNearDistance = WorldWind.WWMath.perspectiveNearDistance(viewport.width, viewport.height, distanceToSurface);
